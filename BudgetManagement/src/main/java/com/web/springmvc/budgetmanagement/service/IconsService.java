...
public class IconsService {
    ...
    public IconsDto getIconById(Long id) {
        return mapToDto(iconRepository
                .findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Not found Icon")));
    }

    public List<IconsDto> getIconsCondition(String type, Long id) {
        if (type != null && id != null) {
            return iconRepository
                    .findByTypeAndIconCategoryId(IconType.valueOf(type), id)
                    .stream()
                    .map(this::mapToDto)
                    .toList();
        } else if (type != null) {
            return iconRepository
                    .findByType(IconType.valueOf(type))
                    .stream()
                    .map(this::mapToDto)
                    .toList();
        } else if (id != null) {
            return iconRepository
                    .findByIconCategoryId(id)
                    .stream()
                    .map(this::mapToDto)
                    .toList();
        } else {
    ...
    private IconsDto mapToDto(Icon icon) {
        ...
        iconsDto.setType(icon
                .getType()
                .name());
        iconsDto.setIconCategoryId(icon
                .getIconCategory()
                .getId());
        return iconsDto;
    }
    ...
    private Icon mapToEntity(IconsDto iconsDto) {
        ...
        icon.setIconCategory(iconCategoryRepository
                .findById(iconsDto.getId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Not found icon category")));
        return icon;
    }
}
